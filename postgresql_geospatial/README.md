...
This cookbook creates and configures postgresql as a database server with environment specific variable settings required for an application specific geospatial database. 
...
should include the `postgresql_geospatial::server` recipe, which will include
...
`postgresql_geospatial::server` recipe, which will include this on RHEL/Fedora
platforms.
