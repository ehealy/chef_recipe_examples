...
when "ubuntu"
  ...
  default['postgresql']['client']['packages'] = %w{postgresql-client libpq-dev postgresql-contrib-9.2}
  default['postgresql']['server']['packages'] = %w{postgresql-9.2 postgresql-contrib-9.2}
  default['postgresql']['contrib']['packages'] = %w{postgresql-contrib postgresql-contrib-9.2}

  pw_creds = Chef::EncryptedDataBagItem.load("database", "geo_user_pw")
  default['postgresql']['geo_user_pw'] = pw_creds['contents']
...
when 'debian'
  ...
  default['postgresql']['config']['shared_buffers'] = '413MB'
  default['postgresql']['config']['wal_level'] = 'hot_standby'
  default['postgresql']['config']['checkpoint_segments'] = '10'
  default['postgresql']['config']['checkpoint_completion_target'] = '0.9'
  default['postgresql']['config']['hot_standby'] = 'on'
  default['postgresql']['config']['max_standby_archive_delay'] = '360s'
  default['postgresql']['config']['max_standby_streaming_delay'] = '360s'
...
default['postgresql']['pg_hba'] = [
  ...
  {:type => 'hostssl', :db => 'geospatial', :user => 'geo_read_only', :addr => '0.0.0.0 0.0.0.0', :method => 'md5'}
]
