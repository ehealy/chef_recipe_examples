...
This cookbook creates and configures postgresql as a database server with environment specific variable settings required for an application specific weather database.
...
should include the `postgresql_weather::server` recipe, which will include
...
`postgresql_weather::server` recipe, which will include this on RHEL/Fedora
platforms.
...
list `recipe[postgresql_weather]` or `recipe[postgresql_weather::client]`.
