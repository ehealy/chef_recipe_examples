...
This cookbook creates and configures postgresql as a master databse server with environment specific variable settings. 
...
should include the `postgresql_server_master::server` recipe, which will include
...
`postgresql_server_master::server` recipe, which will include this on RHEL/Fedora
platforms.
...
list `recipe[postgresql_server_master]` or `recipe[postgresql_server_master::client]`.

On systems that should be PostgreSQL servers, use
`recipe[postgresql_servre_master::server]` on a run list. This recipe does set a
